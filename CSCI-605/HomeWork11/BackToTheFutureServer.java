 ...
 * 
 *  Version: BackToTheFutureServer.java, v 1.0 2017/17/11
 *  
 *  Revisions:
...
public class BackToTheFutureServer extends Thread {

	private static ServerSocket aServerSocket;
	private static String fileNameDictionary;

	public BackToTheFutureServer() {

	}

	public BackToTheFutureServer(int port) {
		try {
	...
	/**
	 * Main code that starts execution of the program
	 * 
	...
	public void run() {
		try {
			...
			// Read the words from client
			ObjectInputStream objectInputStream = new ObjectInputStream( server.getInputStream() );

			// To display information to the client
	...
	public void HangMan( String inputFileName, ObjectInputStream objectInputStream, ObjectOutputStream objectOutputStream) throws IOException, ClassNotFoundException {

		BackToTheFutureServer backToTheFuture = new BackToTheFutureServer();
	...
	private String[] readFromDictionaryFile(String inputFileName) {
		...
		Scanner textFileScanner = new Scanner(BackToTheFutureServer.class.getResourceAsStream(inputFileName));
	...
	private String readAndValidateUserInput( ObjectInputStream objectInputStream ) throws ClassNotFoundException, IOException {
		System.out.println();
		String guess = null;
	...
	public static void main(String[] args) {
		...
		try {
			BackToTheFutureServer backToTheFutureServer = new BackToTheFutureServer(port);
			backToTheFutureServer.run();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
