 ...
 * BalancingScale.java 
 * 
 * Version: BalancingScale.java, v 1.0 2017/06/09
 * 
 * Revisions: 
...
public class BalancingScale {
	...
	static List<Integer> smallestSet = new ArrayList<Integer>();
	static boolean T[][];
				...
				if( i - 1 >= 0 ) {
					if( T[i][j] == true && T[i - 1][j] == true) {
						...
						if( T[i][j] == true && 
							...
							smallestSet.add( arrayOfWeights[i-1] );
	...
	public void printSmallestSet() {
		for( int i = 0; i < smallestSet.size() - 1; i++ ) {
			for( int j = i+1; j < smallestSet.size(); j++) {
				if( canBeBalanced(smallestSet.get(i) + smallestSet.get(j)) ) {
					System.out.println(smallestSet.get(i) + smallestSet.get(j));
					smallestSet.remove(i);
					smallestSet.remove(j);
					Collections.sort(smallestSet);
				}
			}
		}
		for( int i = 0; i < smallestSet.size(); i++ ) {
			System.out.println(smallestSet.get(i));
		}
	}
	...
	public static void main(String args[]) {
		...
		//Case to print the weights
			...
			balancingScale.printSmallestSet();
		}
